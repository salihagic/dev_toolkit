...
extension StringNullableX on String? {
  ...
  bool get containsUrl => RegExp(r'<a\b[^>]*>(.*?)<\/a>', caseSensitive: false).hasMatch(value);

  bool get isIban => RegExp(r'^([A-Z]{2}[ ' + '\\\\' + '-]?[0-9]{2})(?=(?:[ ' + '\\\\' + '-]?[A-Z0-9]){9,30}\$)((?:[ ' + '\\\\' + '-]?[A-Z0-9]{3,5}){2,7})([ ' + '\\\\' + '-]?[A-Z0-9]{1,3})?\$', caseSensitive: false).hasMatch(value);

  String coalesce(String other) => isNotNullOrEmpty ? value : other;

  String capitalize() => count < 2 ? value.toUpperCase() : '${value[0].toUpperCase()}${value.substring(1).toLowerCase()}';
